        ...
        <div className="grid items-center gap-12 lg:grid-cols-12">
          ...
          <div className="lg:col-span-7 text-center lg:text-left">
            ...
            <motion.div
              ...
              className="inline-flex items-center gap-2 rounded-full border border-sky-400/40 bg-sky-500/10 px-4 py-1.5 text-xs font-extrabold tracking-wider text-sky-800 dark:text-sky-300 mb-6 backdrop-blur-md"
            >
            ...
            <motion.h1
              ...
              className="mb-4 text-4xl font-extrabold tracking-tight text-slate-950 dark:text-white sm:text-6xl lg:text-7xl leading-[1.08]"
            >
              TURN YOUR SKILLS <br />
            ...
            >
              <div className="flex flex-nowrap whitespace-nowrap items-center justify-center lg:justify-start gap-1.5 sm:gap-2 text-[11px] sm:text-xs font-extrabold uppercase tracking-wider text-slate-100">
                {['LEARN', 'CREATE', 'SHOWCASE', 'CONNECT', 'FREELANCE'].map((word, index, arr) => (
                  <div key={word} className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                    <span className="relative rounded-xl border border-sky-400/40 bg-sky-500/10 px-2.5 sm:px-3 py-1 text-sky-300 shadow-sm backdrop-blur-md transition-all hover:scale-105 hover:border-sky-400 hover:bg-sky-500/20">
                      {word}
                    </span>
                    {index < arr.length - 1 && (
            ...
            >
              <button
                ...
                className="group relative inline-flex items-center justify-center gap-2 overflow-hidden rounded-full bg-gradient-to-r from-sky-500 via-blue-600 to-teal-400 px-8 py-4 text-xs font-extrabold uppercase tracking-wider text-white shadow-xl shadow-sky-500/25 transition-transform duration-300 hover:scale-105 active:scale-95"
              >
              ...
              <button
                ...
                className="glass-crystal inline-flex items-center justify-center gap-2 rounded-full border border-slate-300 dark:border-slate-700 px-8 py-4 text-xs font-extrabold uppercase tracking-wider text-slate-900 dark:text-white transition-all hover:bg-slate-100 dark:hover:bg-slate-800 hover:scale-105"
              >
            ...
            >
              <button
                ...
                className={`group inline-flex items-center gap-3 rounded-full border px-5 py-2.5 text-xs font-extrabold uppercase tracking-wider transition-all ${
                  isPlayingSequence
              ...
              >
                <div className={`flex h-6 w-6 items-center justify-center rounded-full bg-sky-500 text-white ${isPlayingSequence ? 'animate-spin' : ''}`}>
                  <Play size={12} className="fill-current ml-0.5" />
                </div>
                <span>{isPlayingSequence ? 'SIMULATING FREEVERSE FLOW...' : 'ENTER THE FREEVERSE (INTERACTIVE TRIGGER)'}</span>
              </button>
              ...
              {activeStep !== null && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="mt-3 flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-xs font-bold text-sky-300 shadow-lg backdrop-blur-md border border-sky-400"
                >
          ...
          <div className="lg:col-span-5 relative">
            ...
            >
              <div className="glass-strong relative overflow-hidden rounded-3xl p-6 sm:p-8 border border-sky-300/80 dark:border-sky-500/40 shadow-2xl">
                <div className="mb-6 flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
                  <div className="flex items-center gap-2">
                    <div className="h-3 w-3 rounded-full bg-sky-500 animate-ping" />
                    <span className="text-xs font-extrabold uppercase tracking-widest text-sky-800 dark:text-sky-300">
                      LIVE STUDENT PIPELINE
                    </span>
                  </div>
                  <span className="rounded-full bg-emerald-500/10 px-2.5 py-0.5 text-[10px] font-extrabold text-emerald-700 dark:text-emerald-400 border border-emerald-500/20">
                    ACTIVE
                  </span>
                </div>
                ...
                <div className="space-y-3">
                  {journeySequence.map((item, idx) => {
                    ...
                    return (
                      <div
                        key={item.step}
                        className={`flex items-center justify-between rounded-xl p-3 text-xs font-bold transition-all duration-300 ${
                          isActive
                            ? `bg-gradient-to-r ${item.color} text-white shadow-lg scale-[1.02]`
                            : isPassed
                            ? 'bg-sky-50 dark:bg-slate-800/80 text-sky-800 dark:text-sky-300 border border-sky-300 dark:border-sky-500/30'
                            : 'glass-panel text-slate-700 dark:text-slate-300 hover:border-sky-400/60'
                        }`}
                      >
                        ...
                        <span className="text-[11px] font-semibold opacity-95">{item.label}</span>
                      </div>
                    );
                  })}
                </div>
